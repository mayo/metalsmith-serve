        ...
        fileServer.serve(request, response, function(err, res) {
          if (err) {
            if (options.redirects[request.url]) {
              log(chalk.yellow("[" + REDIRECT_CODE + "] " + request.url + " > " + options.redirects[request.url]), true);
              response.writeHead(REDIRECT_CODE, {"Location": options.redirects[request.url]});
              response.end(HTTPStatus[REDIRECT_CODE]);
            }
