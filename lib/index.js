        ...
        fileServer.serve(request, response, function(err, res) {
          if (err) {
            if (options.redirects[request.url]) {
              log(chalk.yellow("[" + 301 + "] " + request.url + " > " + options.redirects[request.url]), true);
              // fileServer.serveFile(null, 301, {'Location': options.redirects[request.url]}, request, response);
              response.writeHead(301, {"Location": options.redirects[request.url]});
              response.end("Moved Permanently");
            }
            else if (err.status === 404 && options.not_found) {
              log(chalk.yellow("[" + err.status + "] " + request.url + " - served: " + options.not_found), true);
              fileServer.serveFile(options.not_found, 404, {}, request, response);
            }
            else {
